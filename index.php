    ...
    ajax.onreadystatechange = function() {
        if (ajax.readyState === 4) {
            if (ajax.status === 200) {
                ...
                let result = "Сатистика за указанный временной интервал:";
                result += "<table border = 1>";
                result += "<tr><th>Name</th><th>Balance</th><th>Start</th><th>End</th><th>In</th><th>Out</th></tr>";
...
function action3() {
    ...
    if (ajax.readyState === 4) {
        if (ajax.status === 200) {
            ...
            result += "<table border=1>";
            result += "<tr><th>Name</th><th>IP</th><th>Balance</th></tr>";
            for (var i = 0; i < rows.length; i++) {
                result += "<tr>";
                result += "<td>" + rows[i].name + "</td>";
                result += "<td>" + rows[i].IP + "</td>";
